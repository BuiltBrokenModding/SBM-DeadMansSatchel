...
public class SatchelConfiguration {
	...
	public static SatchelGlobalData populateGlobal(String path, ItemDeadMansSatchel satchel, JsonWriter writer, JsonReader reader) {
		try {
								...
								if(reader.nextName().equals("random_bag_drop_chance")) {
									...
									if(reader.nextName().equals("only_owner_use")) {
										...
										if(reader.nextName().equals("per_item_drop_chance")) {
											...
											reader.endArray();
											if(reader.nextName().equals("slot_count")) {
												slotCount = reader.nextInt();
												if(reader.nextName().equals("is_list_blacklist")) {
													isBlacklist = reader.nextBoolean();
													if(reader.nextName().equals("item_list")) {
														reader.beginArray();
														while(reader.hasNext()) {
